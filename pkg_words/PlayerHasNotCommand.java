...
/**
 * Classe de la commande PlayerHasNotCommand
 *
 * @author Luca
 * @version 24/05
 */
public class PlayerHasNotCommand extends Command
{
    
    /**
     * Constructor for objects of class PlayerHasNotCommand
     */
    public PlayerHasNotCommand()
    {
    }
    ...
    public boolean execute(Player aPlayer, UserInterface aGui, GameEngine pGameEngine) {
        if (hasSecondWord() && pGameEngine.getTestMod()) {
            if (aPlayer.getItem(getSecondWord()) == null) {
                ...
                aGui.println("error, this item is in the player inventory : " + getSecondWord());
                return true;
            }
        } else {
